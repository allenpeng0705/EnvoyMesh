 ...
 */
export const ENVOY_TOOL_CATALOG = [
    {
...
export function buildAgentConfig(params) {
    return {
        ...
        permissions: {
            ...
            maxSensitivity: params.permissions.maxSensitivity ?? "public",
        },
        bonds: params.bonds,
        model: {
 ...
 */
export function buildOpenClawSystemPrompt(ownerName, config) {
    const lines = [
    ...
    if (config?.webSearch?.enabled) {
        ...
        lines.push("--- Web search (ENABLED) ---", `Provider: ${provider}. Tool: web_search (also tavily_search when Tavily is configured).`, "You CAN look up current news, headlines, prices, weather, and post-cutoff facts.", "For any time-sensitive question: call web_search with a focused query BEFORE answering.", "Do NOT refuse with training-data cutoff excuses — search first, then summarize results.", "");
    }
    else if (config?.webSearch?.enabled === false) {
        lines.push("--- Web search ---", "Web search is disabled in EnvoyMesh settings. Answer from training data and mesh tools only.", "");
    }
    if (config) {
        ...
        lines.push(`Bond autonomy: ${config.permissions.bondAutonomy ? `ALLOWED (max ${config.permissions.maxBondsPerDay}/day)` : "DENIED"}`);
