...
export const WAN_TWO_NAT_CHECKLIST_STEPS: WanTwoNatChecklistStep[] = [
  {
    id: "relay",
...
export function formatWanTwoNatOperatorChecklist(input?: WanTwoNatSignOffInput): string {
  const lines: string[] = [
  ...
  lines.push("");
  if (input?.relayAddr) lines.push(`Relay: ${input.relayAddr}`);
  if (input?.natAPeerId) lines.push(`NAT A peerId: ${input.natAPeerId}`);
  if (input?.natBPeerId) lines.push(`NAT B peerId: ${input.natBPeerId}`);
  if (input?.automatedBaselineOk) lines.push("Automated baseline: [x] wan-relay-signoff-e2e");
  if (input?.chatVerified) lines.push("Manual two-NAT chat: [x] verified");
  if (input?.operator) lines.push(`Operator: ${input.operator}`);
