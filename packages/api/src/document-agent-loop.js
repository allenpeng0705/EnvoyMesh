...
export function classifyDocumentIntent(message) {
    ...
    const requestFromMatch = text.match(/^request share from\s+(.+?)\s+for\s+(.+)/i) ??
    ...
    const discoverMatch = text.match(/^(?:discover|find on (?:the )?(?:mesh|contacts|network))\s+(.+)/i) ??
...
function extractQuotedOrPath(text) {
...
export function matchLibraryItem(items, hint) {
    if (!hint?.trim())
        return undefined;
    const h = hint.trim().toLowerCase();
    return (items.find((i) => i.documentId === hint) ??
...
function formatLibraryList(items) {
    if (items.length === 0) {
...
function formatDiscoverResult(peers) {
    ...
    const parts = ["Published file metadata from your contacts (bytes require share accept):"];
    for (const p of rows) {
        if (p.error) {
        ...
        for (const f of p.files) {
            ...
            parts.push(`• ${name} (${p.bondLevel}): ${f.title} — ${f.relativePath} hash ${f.contentHash.slice(0, 12)}…`);
...
export async function runDocumentAgentTurn(deps) {
    ...
    if (intent.kind === "transfer_status") {
        ...
        const lines = transfers.map((t) => `• ${t.correlationId.slice(0, 12)}… ${t.phase}${t.vaultRelativePath ? ` — ${t.vaultRelativePath}` : ""}`);
        return {
...
//# sourceMappingURL=document-agent-loop.js.map