...
import { buildOwnerDidPresentation, deriveDidKeyFromEd25519PublicKey, didKeysMatch, ed25519RawPublicKeyFromSpkiPem, parseDidLookupInput, } from "./owner-did-presentation.js";
...
export function ed25519RawPublicKeyFromDidKeyMultibase(multibase) {
...
export function ed25519SpkiPemFromRawPublicKey(rawPublicKey) {
...
function resolveFromPublicKeyPem(publicKeyPem, didHint, document) {
    try {
...
export function parseExternalDidDocumentJson(raw) {
    let parsed;
    try {
    ...
    const parsedDid = docId.startsWith("did:key:") ? parseDidLookupInput(docId) : { kind: "invalid" };
    ...
    const multibase = typeof vm?.publicKeyMultibase === "string"
...
export function resolveDidImportInput(raw) {
...
export function verifyDidKeyMatchesPublicKeyPem(did, publicKeyPem) {
    const parsed = parseDidLookupInput(did);
    if (parsed.kind !== "did-key" || !parsed.did)
        return false;
    try {
...
//# sourceMappingURL=did-import.js.map