 ...
 */
export function wireChannelForIntent(intent: string): EnvoyWireChannel {
  if (CHAT_PROTOCOL_INTENTS.has(intent)) return "chat";
  if (DATA_PROTOCOL_INTENTS.has(intent)) return "data";
  return "message";
}
...
export function isH2aWireIntent(intent: string): boolean {
  return H2A_WIRE_INTENTS.has(intent);
}

export function isPeerHumanChatIntent(intent: string): boolean {
