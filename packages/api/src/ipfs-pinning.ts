...
export type PinCidResult =
  | { ok: true; provider: IpfsPinningProvider; pinId?: string }
  | { ok: false; error: string };

const PROVIDER_ENV: Record<IpfsPinningProvider, string> = {
  pinata: "ENVOYMESH_PINATA_JWT",
  web3storage: "ENVOYMESH_WEB3_STORAGE_TOKEN",
};

async function pinViaPinata(cid: string, name: string | undefined, jwt: string): Promise<PinCidResult> {
...
async function pinViaWeb3Storage(cid: string, token: string): Promise<PinCidResult> {
  ...
  let pinId: string | undefined;
  try {
    const body = (await response.json()) as { requestid?: string; cid?: string };
...
export async function pinCidToProvider(input: PinCidInput): Promise<PinCidResult> {
