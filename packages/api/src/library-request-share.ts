...
export async function runLibraryRequestShare(
  deps: LibraryRequestShareDeps,
  input: LibraryRequestShareInput,
): Promise<{ ok: true; result: LibraryRequestShareResult } | { ok: false; error: string }> {
  ...
  const fileTitleQuery =
    input.fileTitleQuery?.trim() ||
  ...
  const matchLines =
    matches.length > 0
      ? matches
          .map(
            (f) =>
              `• ${f.title} (${f.relativePath}, hash ${f.contentHash.slice(0, 12)}…)`,
          )
          .join("\n")
