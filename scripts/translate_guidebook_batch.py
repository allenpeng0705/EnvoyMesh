...
PRESERVE_TERMS = [
    ...
    'Social', 'LiteLLM', 'Anthropic', 'OpenAI', 'Filecoin', 'Node.js',
    ...
    'log', 'shell', 'remote', 'exposure', 'privilege', 'restrict',
...
def translate_file(input_file, output_file):
    ...
    for i, section in enumerate(sections):
        if section.startswith('#'):
            ...
            if header_text in SECTION_TRANSLATIONS:
                translated_header = SECTION_TRANSLATIONS[header_text]
            else:
                translated_header = translate_paragraph(header_text)
            
            prefix = header[:len(header) - len(header_text)].strip()
            translated_sections.append(f'{prefix} {translated_header}')
        else:
            if section.strip():
                translated_sections.append(translate_paragraph(section))
            else:
                translated_sections.append(section)
        
        if (i + 1) % 10 == 0:
            print(f'Translated {i + 1}/{total_sections} sections...')
        time.sleep(0.02)
