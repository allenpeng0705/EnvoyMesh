...
/**
 * The constant is generated twice: `@envoymesh/protocol` **declares** it (so a
 * reusable package like `harness` can read the version without importing the
 * product surface, which would classify it `product-bound`), and
 * `@envoymesh/api` re-exports it so its public surface is unchanged.
 */
function generateVersionTs(filePath, { reexport = false } = {}) {
  const header =
    `// AUTO-GENERATED by scripts/sync-version.mjs — do not edit manually.\n` +
    `// Source of truth: VERSION file at repo root.\n`
  const content = reexport
    ? header +
    ...
    : header + `export const ENVOYMESH_VERSION = ${JSON.stringify(version)} as const;\n`
  try {
...
generateVersionTs(join(ROOT, "packages", "protocol", "src", "version.ts"))
generateVersionTs(join(ROOT, "packages", "api", "src", "version.ts"), { reexport: true })
