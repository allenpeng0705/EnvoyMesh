#!/usr/bin/env node
/**
 * Generate the reusable surface of `@envoymesh/api`:
 *
 *   1. `packages/api/src/core-node-service.ts` — the reusable half of the RPC
 *      contract (plan §10 **E9**, reopened in §8.13).
 *   2. `packages/api/src/core.ts` — the package's declared `./core` entry point:
 *      the barrel's re-exports filtered to modules the Axis-1 manifest calls
 *      `reusable`. Declared subpath entry points are their own surface to the
 *      classifier, so `@envoymesh/api/core` can be a core package while
 *      `@envoymesh/api` keeps tainting — which is what lets a reusable module
 *      import the contract without importing the product surface.
 *
 * ## What this produces
 *
...
const CORE_PACKAGES = manifest.declaredInputs?.corePackages ?? [];
...
// ── emit the `./core` entry point ───────────────────────────────────────────

/**
 * `api/core` mirrors the barrel's re-export statements, keeping only those whose
 * target module the manifest calls `reusable`.
 *
 * Generated from `index.ts` rather than by globbing `src/*.ts` on purpose: the
 * barrel is where re-export *conflicts* are already resolved (an explicit
 * `export { x } from "./a.js"` winning over an `export *`), so filtering it
 * inherits that resolution instead of rediscovering it as a TS2308 error.
 */
const API_BARREL = path.join(API_SRC, "index.ts");
const CORE_ENTRY = path.join(API_SRC, "core.ts");
const barrelText = await read(API_BARREL);

const exportStatements = [];
{
  const re = /^export (?:\*|type \{[^}]*\}|\{[^}]*\}) from "([^"]+)";$/gms;
  for (const m of barrelText.matchAll(re)) {
    const stmt = m[0];
    const spec = m[1];
    if (!spec.startsWith(".")) {
      // A re-export from another package: core when that package is declared
      // core (the symbol then cannot reach product code by definition), and it
      // has to be carried here or a repointed module's import disappears.
      const root = spec.split("/").slice(0, 2).join("/");
      const isCorePkg = CORE_PACKAGES.includes(spec) || CORE_PACKAGES.includes(root);
      exportStatements.push({ stmt, target: spec, reusable: isCorePkg });
      continue;
    }
    const target = `packages/api/src/${spec.slice(2).replace(/\.js$/, "")}.ts`;
    exportStatements.push({ stmt, target, reusable: reusable.has(target) });
  }
}

const coreExcluded = exportStatements.filter((e) => !e.reusable).length;
const coreEmitted = exportStatements.filter((e) => e.reusable);
const coreHeader = `/**
 ...
 * Axis-1 manifest calls \`reusable\` (${coreEmitted.length} of the barrel's ${exportStatements.length} re-export
 * statements; ${coreExcluded} product-bound modules are deliberately absent — \`NodeService\`
...
const coreEntry = [coreHeader, ...coreEmitted.map((e) => e.stmt), ""].join("\n");
...
if (flag("--check")) {
  ...
  let coreOnDisk = null;
  try { coreOnDisk = await read(CORE_ENTRY); } catch { /* missing */ }
  if (coreOnDisk === null) {
    console.error(`[fail] ${rel(CORE_ENTRY)} is missing — run node scripts/generate-core-surface.mjs`);
    process.exit(1);
  }
  if (coreOnDisk !== coreEntry) {
    console.error(`[fail] ${rel(CORE_ENTRY)} is stale — re-run node scripts/generate-core-surface.mjs`);
    console.error("       (a module was classified, added to, or removed from the api barrel)");
    process.exit(1);
  }
  ...
  console.log(
    `core-surface OK — ${coreMemberNames.length} core methods over ${coreSections.length} declared sections; ` +
      `api/core re-exports ${coreEmitted.length} reusable modules (${sectionOrder.length} sections total)`,
  );
  process.exit(0);
}
...
await fs.writeFile(CORE_ENTRY, coreEntry);
console.log(
  `wrote ${rel(CORE_ENTRY)} — ${coreEmitted.length} reusable modules re-exported ` +
    `(${coreExcluded} product-bound modules excluded)`,
);
