...
**版本类型：** 完整指南版  
**修订日期：** 2026-08-27  
**语言：** [English](EnvoyMesh_GuideBook_0.3.0.md) · [简体中文](EnvoyMesh_GuideBook_0.3.0.zh-CN.md) ([HTML](EnvoyMesh_GuideBook_0.3.0.html) · [中文 HTML](EnvoyMesh_GuideBook_0.3.0.zh-CN.html))  
...
> **完整指南版本。** 本指南反映了修订日期时 EnvoyMesh 0.3.0 存储库的状态。它是为最终用户编写的，而不是作为内容大纲存根。功能状态可能因平台和部署而异 — 在生产环境中依赖它之前，请验证您构建中的每个 Beta 或实验性功能（发行说明、设置标签和附录 J）。
...
本附录补充完整英文指南（`EnvoyMesh_GuideBook_0.3.0.md`）中 0.3.0 版本新增的小节要点。详细步骤请参阅英文版对应章节编号。
...
桌面默认使用主节点上的 **Envoy Local** 嵌入 sidecar（Qwen3-Embedding），无需云端嵌入 API 密钥。若块过大或 sidecar 卡住，**重新索引**会自动缩小并重试、必要时重启嵌入进程，仅跳过仍失败的文档（英文指南 §29.10–29.11）。升级到 0.3.0 后请运行一次 **知识库 → 重新索引**。
