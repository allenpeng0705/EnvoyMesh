...
export async function cancelChainOwnerAction(
  ...
  const notifyPeers = new Set<string>();
  for (const id of cancelled) {
    const award = state.awards.get(id);
    if (award?.workerPeerId) notifyPeers.add(award.workerPeerId);
  }
  if (notifyPeers.size > 0) {
    try {
      const orchDeps = await buildChainOrchestratorDeps(deps);
      const nowIso = new Date().toISOString();
      for (const peer of notifyPeers) {
        await sendChainCancel(orchDeps, peer, {
          chainId: params.chainId,
          subtaskId: params.subtaskId,
          reason: params.reason.slice(0, 2000),
          cancelledBy: params.cancelledBy,
          notifyWorkerPeerIds: [...notifyPeers],
          createdAt: nowIso,
        });
      }
    } catch (err) {
