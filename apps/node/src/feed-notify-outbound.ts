  ...
  for (const ownerId of recipientOwnerIds) {
    try {
      ...
      if (!resolved?.peerId || !isLibp2pPeerId(resolved.peerId)) {
        console.warn(`[feed.notify] skip bond ${ownerId.slice(0, 20)}…: no libp2p peer id`);
        continue;
      }
      ...
      try {
        ...
        console.warn(
          `[feed.notify] dial hints failed for ${ownerId.slice(0, 16)}…:`,
      ...
      try {
        ...
        console.warn(
          `[feed.notify] send to ${ownerId.slice(0, 16)}… failed after retry:`,
          err instanceof Error ? err.message : err,
        );
      }
    } catch (bondErr) {
      console.warn(
        `[feed.notify] bond ${ownerId.slice(0, 20)}… failed:`,
        bondErr instanceof Error ? bondErr.message : bondErr,
      );
    }
  }

  return { attempted: recipientOwnerIds.length, sent };
}
