...
export async function getNodeConfigViaRuntime(
  ...
  if (config) {
    ...
    return {
      ...
      relayRosterUrl: config.relayRosterUrl,
      relayRosterTrustKeys: config.relayRosterTrustKeys,
      relayRosterEnabled: config.relayRosterEnabled,
      relayRosterPollMs: config.relayRosterPollMs,
      modelProviders,
      ...
      aiSettings: normalizeAiSettings(config.aiSettings),
