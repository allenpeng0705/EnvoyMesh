...
describe("ToolRegistry", () => {
  ...
  describe("listTools", () => {
    it("lists all registered tools including defaults", () => {
      ...
      // 20 default tools + 2 additional = 22
      expect(tools).toHaveLength(22);
      expect(tools.map((t) => t.name).sort()).toEqual(
        [
          "bond.send_hello", "chat.send", "discovery.search", "knowledge.query",
          "mesh.acknowledge-escalation", "mesh.get-contact-disclosure", "mesh.get-external-agent",
          "mesh.get-mode", "mesh.get-style", "mesh.list-external-agent-actions",
          "mesh.list-external-sessions", "mesh.list-sessions", "mesh.revoke-external-agent",
          "mesh.session-summary", "mesh.set-contact-disclosure", "mesh.set-contact-mode",
          "mesh.set-mode", "mesh.set-style", "share.send", "tool.1", "tool.2",
          "vault.search",
        ].sort(),
      );
    });
    ...
    it("default tools are pre-registered", () => {
      const registry = new ToolRegistry();
      const tools = registry.listTools();
      // Default tools: 6 core + 4 gateway management + 3 mode + 3 session + 4 style = 20
