/**
 * Agent knowledge.query over real libp2p — requires verifyInboundEnvelope + owner resolution from agent credential.
 */
import {
...
async function createTestNode(): Promise<TestNode> {
  const profileDir = await mkdtemp(join(tmpdir(), "envoymesh-kq-agent-"));
...
describe("E2E knowledge.query (agent over libp2p)", () => {
  it("executeTool knowledge.query receives knowledge.response on same stream", async () => {
    ...
    await writeFile(
      join(bob.vaultDir, "docs/network-guide.md"),
      "EnvoyMesh is a decentralized P2P network for AI agents and humans.",
      { mode: 0o600 },
    );
    ...
    expect(envelope.intent).toBe("knowledge.response");
    const payload = parseKnowledgeResponsePayload(envelope.payload);
    expect(payload.refused).not.toBe(true);
