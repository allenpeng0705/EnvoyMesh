/**
 * P0 #2: tier-2 document autonomy triggers real shareFile over libp2p (not Inbox proposal).
 */
import {
...
async function createTestNode(): Promise<TestNode> {
  const profileDir = await mkdtemp(join(tmpdir(), "envoymesh-doc-autonomy-e2e-"));
...
describe("E2E document autonomy over libp2p (ADB-F)", () => {
  it("tier 0 share intent creates Inbox proposal without sending share.request", async () => {
    ...
    const content = "tier 0 proposal only\n";
  ...
  it("tier 2 share intent auto-shares over libp2p to bonded contact", async () => {
    const bob = await createTestNode();
    const alice = await createTestNode();
    await registerBondedPeer(bob, alice, "Alice");
    ...
    const content = "tier 2 autonomous share e2e\n";
    await mkdir(join(bob.vaultDir, "out"), { recursive: true });
    ...
    expect(turn.intent).toBe("share_propose");
    expect(turn.toolsUsed).toContain("mesh.share_propose");
    expect(await bob.service.listAgentShareProposals()).toHaveLength(0);
    ...
    expect(offers[0]?.senderVaultRelativePath).toContain("autonomy");

    await alice.service.acceptShare(offers[0]!.shareId, "inbox/autonomy-received.txt");

    await waitFor(async () => {
      try {
        const out = await readFile(join(alice.vaultDir, "inbox/autonomy-received.txt"), "utf8");
        return out === content;
      } catch {
        return false;
      }
    ...
    const received = await readFile(join(alice.vaultDir, "inbox/autonomy-received.txt"), "utf8");
