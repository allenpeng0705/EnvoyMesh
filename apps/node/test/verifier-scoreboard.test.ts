...
describe("VerifierScoreboard", () => {
  it("appends owner-signed entries and reads them back oldest-first", async () => {
    const board = new VerifierScoreboard({ filePath: join(dir, "scoreboard.jsonl"), ownerPublicKeyPem: owner.publicKeyPem })
    const e1 = entry()
  ...
  it("rejects entries not signed by the owner", async () => {
    ...
    const bad = { ...unsigned, ownerSignature: signCanonicalPayload(unsigned, other.privateKeyPem) } as VerifierScoreboardEntry
  ...
  it("is readable when the file is missing (fresh node)", async () => {
    const board = new VerifierScoreboard({ filePath: join(dir, "does-not-exist.jsonl"), ownerPublicKeyPem: owner.publicKeyPem })
