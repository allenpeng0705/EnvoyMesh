...
void main() {
  group('PhoneSocialBackend discovery lifecycle seam', () {
    test('wanSearch attach then clear on stop-shaped sequence', () async {
      ...
      // Mimic PhoneDiscoverySession.stop()
  ...
  group('LAN hit merge (S7)', () {
    test('mergeHits keeps LAN multiaddrs and prefers owner-bearing hits', () {
      ...
      expect(merged.single.multiaddrs, contains('/ip4/192.168.1.5/tcp/4001/p2p/12D3KooLan'));
      expect(
    ...
    test('LAN-only peer appears when no owner yet', () {
      const lan = MeshPeerHit(
        nodeId: '12D3KooOnlyLan',
        ownerId: '',
        multiaddrs: ['/ip4/10.0.0.8/tcp/4001/p2p/12D3KooOnlyLan'],
        interests: ['lan'],
      );
      final merged = PhoneDiscoveryRuntime.mergeHits(
        [lan],
      ...
      expect(merged.single.nodeId, '12D3KooOnlyLan');
  ...
  group('capability topic CID', () {
    test('matches desktop golden music CID', () {
      expect(
