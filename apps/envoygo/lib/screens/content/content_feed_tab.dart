...
class _ContentFeedTabState extends ConsumerState<ContentFeedTab> {
  ...
  void initState() {
    ...
    ref.listenManual(nodeProvider.select((s) => s.connectionState), (
      prev,
      next,
    ) {
      if (prev != NodeConnectionState.connected &&
          next == NodeConnectionState.connected) {
        _refresh();
        _loadSelfProfile();
      }
    });
  ...
  String _nameFor(BuildContext context, String ownerId, {bool isOwn = false}) {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          content: Text(l10n.feedDeleteBody),
          actions: [
            TextButton(
  ...
  Widget _avatar(BuildContext context, _TimelineItem item, String name) {
    final selfId = ref.read(nodeProvider).ownerId?.trim();
    final isSelf =
        item.isOwn || (selfId != null && item.publisherOwnerId == selfId);
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) =>
                ProfileScreen(ownerId: isSelf ? null : item.publisherOwnerId),
          ),
        );
      },
      child: ProfileAvatar(
  ...
  Widget _feedCard(BuildContext context, _TimelineItem item) {
    final scheme = Theme.of(context).colorScheme;
    final l10n = AppLocalizations.of(context);
    final name = _nameFor(context, item.publisherOwnerId, isOwn: item.isOwn);
              ...
              SliverPadding(
                ...
                sliver: SliverList.separated(
                  ...
                  itemBuilder: (context, index) =>
                      _feedCard(context, timeline[index]),
                ),
              ),
