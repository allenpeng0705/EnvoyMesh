...
**Revised:** 2026-08-27  
**Languages:** [English](EnvoyMesh_GuideBook_0.3.0.md) · [简体中文](EnvoyMesh_GuideBook_0.3.0.zh-CN.md) ([HTML](EnvoyMesh_GuideBook_0.3.0.html) · [中文 HTML](EnvoyMesh_GuideBook_0.3.0.zh-CN.html))  
...
> **Complete Guidebook Edition.** This guide reflects the EnvoyMesh 0.3.0 repository state at the revision date. It is written for end users, not as a content-outline stub. Feature status can differ by platform and deployment—verify each Beta or Experimental capability in your build (release notes, Settings labels, and Appendix J) before relying on it in production.
...
If a chunk is too large or the embed sidecar wedges during indexing, **Reindex** recovers automatically: oversized text is shrunk and retried, the embed process may restart, and only documents that still fail are skipped so the rest of your vault stays searchable. After upgrading to 0.3.0, run **Knowledge → Reindex** once to rebuild older chunks under safer size limits. Progress and failure counts appear in the Knowledge UI and audit log.
...
Gather version, platform, profile path, reproduction steps, and redacted audit excerpts with correlation IDs. State which feature status label applies (Available, Beta, Experimental). Community channels are announced in release notes for 0.3.0—avoid posting secrets in public threads.
...
**Available (0.3.0)** — intended for current use on supported platforms:
