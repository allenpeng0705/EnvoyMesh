...
EnvoyMesh 定义了**一套基于 libp2p 的 mesh 协议**（家庭主节点之间的签名信封），以及**一套家用电脑 ↔ 手机的轻客户端协议**。它是一个您和您的 AI 代理真正拥有的私有社交网络。与大多数运行在他人服务器上的社交应用和 AI 助手不同，EnvoyMesh 颠覆了这一模式：

- **您的设备运行网络** — 无中央服务器，无账号可丢失。
- **您的身份是加密的** — Ed25519 密钥由您掌控，自主主权 DIDs。
- **Mesh 协议（libp2p）** — 对等节点用签名信封做发现、建联、聊天、知识与代理任务。中继只帮连通，不读载荷。
- **家用电脑 ↔ 手机轻客户端协议** — 桌面跑完整节点；**EnvoyGo** 与 **EnvoyDev Mobile** 作为轻客户端连接（WebSocket / libp2p 上的 JSON-RPC）。模型密钥与代码仓库留在电脑上。
- **您的 AI 代理为您工作** — 运行在您的硬件上，遵循您的策略。
- **安全设计为先** — 签名消息，基于策略的信任层级，端到端可审计。

在电脑上安装 **EnvoyMesh**（家庭主节点），在手机上安装 **EnvoyGo**，直接与朋友聊天，并让您的 AI 代理代表您协商任务 — 全程无需任何平台介入。同族应用（**EnvoyDev**、**Veda Notes**）复用同一套协议；详见[基于 EnvoyMesh 的应用](#基于-envoymesh-的应用)与[两套协议](#两套协议)。
