...
- **Fixed** (now passing):
  ...
  - **Relay-E2E un-skip (2026-07-09)** -- 4 relay-dependent E2E files now default to the community relay at `47.93.11.212:4001` and run via `RUN_E2E=1 npx vitest run apps/node/test/<file>.test.ts`:
    ...
    - `geo-discovery-wan-signoff.test.ts` -- 1 pass (~226 s) followed by 2 consecutive failures at the 180 s `WAN_TIMEOUT_MS` ceiling. Re-skipped 2026-07-10 — see "Still skipped" entry below.
    ...
    - `relay-broadcast-e2e.test.ts` -- defaults to community cn-relay (same pattern as relay-chat). Re-verify after cn-relay deploys that include circuit-relay-v2 + broadcast fanout.
...
- **relay-broadcast-e2e** (whole describe, 2 tests): green against a local `apps/relay` with the fanout fixes in this tree (`getConnectedPeerIds` + forward original signed envelope). Against cn-relay, re-deploy that relay build then re-run; older cn-relay builds used `getConnectedRelayPeerIds` and mutated signed fields (signature break). Suite defaults to community cn-relay like `relay-chat-e2e`.
- **geo-discovery-wan-signoff** (whole describe, 2 tests): flaky against the community DHT, **skipped-by-default 2026-07-10** (gated on `GEO_WAN_DISABLE_GATE=0` opt-in). The community DHT for the `geo:city:US-geo-signoff` test-only topic is loaded with publishers from many concurrent runs across operators; the advertiser's `provideCapabilityTopic` succeeds (~90 s) but the searcher's `searchPeers` lookup times out against the live DHT. Pattern: 1 pass (~226 s) followed by 2 consecutive failures at the 180 s `WAN_TIMEOUT_MS` ceiling in `searchPeers`. Environmental load, not a code bug — `provideSelf` is fine, the rendezvous lookup just gets dropped by the busy DHT. See `apps/node/test/geo-discovery-wan-signoff.test.ts` header for the opt-in run instructions; with the gate off against a private relay running a fresh DHT the test should be near-deterministic.
