...
>
> **Known gaps (by design):** (1) **Verdict write path** — the authoritative writer is now the orchestrator's verification flow (Sprint 3, `chain-verify-loop.ts` → `writeVerdictEntry` into the `ArbitrationStore`); worker-side `adapter.verify` stays advisory per §7.1 (no self-reported reputation), so the 3-tuple readers and `requiresReputationApproval` go live as verdicts accumulate. (2) ~~Manifest pool consumer~~ **resolved 2026-08-18 (first cut)** — `adapter.manifest` broadcasts land (`agent-adapter-broadcast.ts` + `handleInboundCapabilityManifest` → `ChainSideState.remoteManifests`), and `findAgentNetworkWorkersRanked` prefers a fresh wire manifest over card synthesis, so `findWorkersWithManifests`/`resolveWorkerPool` are consulted with real wire data. Hardening (owner-signature verification on first receipt, manifest TTL sweep) is **also done 2026-08-18** (`pruneExpiredManifests` on inbound + read path).
...
- **Verification-loop wiring (`apps/node/src/chain-verify-loop.ts`) — first cut done 2026-08-18**: `runChainVerificationLoop` is called from `handleOrchestratorPartial`; writes `rule`/`cross` `VerdictEntry`s to the `ArbitrationStore`; escalates to a second runtime on demand. **Production second-doctor seam closed 2026-08-18:** `buildChainOrchestratorDeps`' `chainVerify` now registers both local runtimes in `listRuntimes` and builds the real adapters in `buildAdapter` (`OpenClawAdapter` for `openclaw`; `createPiAdapterFromHost` → local Pi runtime for `pi`, both with the `buildSubtaskPromptForAdapter` prompt surface), so an OpenClaw worker escalates to the local Pi second doctor. Covered by `apps/node/test/second-doctor-pi.test.ts` (production `buildChainOrchestratorDeps` wiring + real-Pi escalation through `handleOrchestratorPartial`).
...
- **Owner UI: mark chain as `criticality: 'high'` in the chain proposal — done 2026-08-18**: `criticality?: "normal" | "high"` on `ChainStartFromGoalParams`, carried on the owner-signed `ChainMandate` (and the whole-job handoff payload), read by `runChainVerificationLoop` from `state.chainMandate.criticality`. The Social `ChainStartDialog` Job settings expose a "High-criticality job" toggle.
- **Manifest broadcast (`apps/node/src/agent-adapter-broadcast.ts`) — first cut done 2026-08-18**: `adapter.manifest` intent added to the protocol (agent→agent); the node builds an owner-signed `SignedCapabilityManifest` (`buildSignedCapabilityManifest`) and pushes it to bonded peers every TTL/2 via `startManifestBroadcaster` (started at node startup). Inbound: `handleInboundCapabilityManifest` verifies the owner signature against the contact-owner key store and stores fresh manifests in `ChainSideState.remoteManifests`; `findAgentNetworkWorkersRanked` now prefers a fresh wire manifest over card synthesis — the manifest-carrying worker pool is **live**. **Hardening done (same day):** the remaining hardening items are closed — owner-signature verification is enforced on *every* receive against the contact-owner key store (unknown key ⇒ dropped, never trusted unverified), and the store is kept bounded by `pruneExpiredManifests` (run on inbound and on the worker-pool read path), so expired TTLs are swept and stale capability claims can never influence ranking.
...
| `packages/api/src/ws-protocol.ts` (`ChainStartFromGoalParams`) | Add `criticality?: "normal" \| "high"` owner flag (Sprint 3, §8.1 #1) — **done 2026-08-18**; carried on the signed `ChainMandate` (`UnsignedChainMandateSchema.criticality`, `packages/protocol/src/agent-network.ts`) and the whole-job handoff payload (`ChainHandoffRequestPayloadSchema`) |
