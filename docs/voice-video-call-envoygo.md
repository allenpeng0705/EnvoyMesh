...
| **Social UI `createWebRtcCallTransport` caller** | Defined at `apps/social/src/lib/webrtc-call-transport.ts:57-251` | Zero callers in `apps/social/src` — the file is dead code until something wires it up |
...
- **NG6.** TURN server provisioning in the relay binary (the "TURN via libp2p relay" open question from Phase 38 stays open). For Path 2 cross-NAT, rely on the user's `iceServers` config in node-config.
- **NG7.** AI agent as a call participant (already a Phase 39 / Phase 38 open question).
- **NG8.** iOS background audio survival beyond what `flutter_webrtc` + iOS `AVAudioSession` defaults provide. Phase 42F documents the minimal config; persistent backgrounded calling is a separate scope.
...
`iceServers` lives on the home in `node-config.json` (`apps/node/src/node-config-store.ts:188-192`) and is already loaded into the `NodeServiceImpl` config snapshot at `node-service-impl.ts:7813`. It needs to reach two consumers:

1. **The home's `sendCallInvite` (42A)** — embed in `call.invite` payload. Read from `this._nodeConfig?.iceServers ?? defaultIceServers` (Google STUN + Cloudflare STUN).
...
This means the user configures `iceServers` **once on the home** in the Social Settings UI (`apps/social/src/components/views/SettingsNodeTab.tsx:67-75`), and it propagates to the phone via the call envelope automatically. No new UI on the phone.
...
4. **Real-device manual smoke (deferred to 42H, same shape as Phase 38H):**
   - Two iOS devices on the same LAN, place a call via EnvoyGo, verify audio.

**Exit criteria:**
- 42G is green. 42H (manual device smoke) is optional and tracked the same way as Phase 38H.
...
| File | Change |
|---|---|
| `apps/node/src/node-service-impl.ts:11913-11961` | Rewrite `sendCallInvite` (42A) and the four response methods (42B). Add the `iceServers` default helper. Add `validateSdpString` / `validateIceCandidate` helpers used by 42A. |
| `apps/node/src/call-inbound.ts` | Add the defensive SDP / ICE candidate validation in `handleCallInvite` / `handleCallAccept` / `handleCallIceCandidate` (42A). |
| `apps/envoygo/lib/services/node_service_client.dart:376-408` | Replace `UnimplementedError` stubs with real JSON-RPC implementations (42C). |
| `apps/envoygo/lib/providers/call_provider.dart` | Add `WebRtcCallTransport` plumbing (42E). Extend `CallState` with `remoteStream` and `transport`. |
...
| `apps/envoygo/ios/Runner/Info.plist` | Add `NSMicrophoneUsageDescription` (42F). |
...
- `apps/social/src/lib/webrtc-call-transport.ts` — unchanged; Phase 38 already shipped the desktop side.
...
|---|---|---|
| Path 1 (libp2p data channel) on the phone | Not in Phase 42 | `flutter_webrtc` does not support binding to a libp2p stream; documenting as NG4 |
...
| `flutter_webrtc` version | Bump to `^0.14.0` if currently `^0.12.0` | Verify against `pubspec.yaml` in 42A before writing code; 0.12 may be fine |
| Background audio on iOS | `AVAudioSession` `playAndRecord` + `voiceChat` mode | Standard config; persistent backgrounded calling is NG8 |
...
| Test | Scope | Phase |
|---|---|---|
| `sendCallInvite` resolves owner ID → device peer ID + embeds SDP + injects `iceServers` | apps/node | 42A |
...
| `WebRtcCallTransport` compiles, lifecycle correct, muting works | apps/envoygo | 42D |
...
| Two iOS devices on the same LAN can place a call (manual) | device | 42H (deferred) |
...
|---|---|---|
| `flutter_webrtc` version on EnvoyGo | Open | `pubspec.yaml:28` declares `^0.12.0`; 42A verifies whether 0.14 is needed for the API surface used. |
| Should the `iceServers` defaults be Google STUN only, or Google + Cloudflare + Twilio as Phase 38 §6.3 recommends? | Open — defaults in 42A | Default to the three-list per Phase 38; user can override in node-config. |
...
| TURN server for symmetric NAT | Open | Phase 38 open question, stays open. 42A documents the workaround: configure public TURN in `iceServers`. |
| Persistent backgrounded calling on iOS | Open | NG8. Documented in 42F. |
...
| 42D | ~250 (Dart transport) | ~150 (mostly fake-`RTCPeerConnection`) | |
| 42E | ~150 (`CallProvider` rewiring) | ~200 | |
| 42F | ~100 (UI binding + iOS/Android config) | ~50 | Manual smoke deferred to 42H |
| 42G | ~80 (Playwright) + ~100 (jsdom two-callManager) | covered above | Reuses existing Phase 38 Playwright setup |
| **Total** | **~890 + ~1,200 tests** | | Comparable to one slice of Phase 40 |

---

## 13. Out-of-scope (parked until scoped)

- **TURN server in the relay binary** — Phase 38 open question. Required for symmetric-NAT phone-to-phone calls.
...
- `apps/node/src/call-inbound.ts` — the home-side trust + identity gate
