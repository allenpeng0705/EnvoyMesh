...
**Active:** **Phase 40 — Agent Network Collaboration Layer.** Design approved in [agent_network.md](./agent_network.md). Bridges the gap between the existing single-shot A2A (Phase 24) and the user's target of multi-agent teams negotiating and collaborating concurrently on complex tasks. Adds the `task.chain.*` wire namespace, parent/child task lineage, a structured `composite` artifact, multi-round counter-proposal / split / merge negotiation, and a `ChainBudgetLedger` so the orchestrator cannot over-commit its signed chain budget across parallel workers. Five sub-phases (40A–40E); 40A is the foundation sprint, 40E (cross-home chains) is explicitly deferred until Phase 11 mobile parity ships. See the Phase 40 section for the full sub-phase layout.
...
## Phase 40 — Agent Network Collaboration Layer (design + implementation) **`[ ]` designed — awaiting owner approval to start 40A`**

> **Status: `[ ]` design approved (see [agent_network.md](./agent_network.md)), implementation not yet started.**
> **Five sub-phases (40A–40E).** 40A is the foundation sprint (protocol + role policy + store extensions, no wire traffic). 40B ships the orchestrator + worker runtime + RPC plumbing for a single-orchestrator 3-worker chain. 40C adds the Social + EnvoyGo UI. 40D adds multi-bid collection + counter-bid UI + LLM decomposer (replaces the keyword fallback). **40E (cross-home chains) is explicitly deferred** until Phase 11 mobile relay parity ships — cross-home routing on an unstable mobile surface is too much area at once.
...
- `[ ]` **Multi-bid:** flip the default `requireMultipleBids` to `true` for chains with > 3 workers. Orchestrator waits for ≥2 bids; window = `min(bidderCount, 3) × 30s`. Single-bidder fallback after the window expires.
- `[ ]` **Counter-bid UI:** extend the orchestrator's bid inbox (new component `apps/social/src/components/ChainBidInbox.tsx`, ~200 lines) so the owner can see incoming bids, review counter-proposals, and accept/reject manually before the orchestrator's auto-evaluation kicks in. Adds an explicit `awaitingOwnerReview` chain state distinct from `negotiating`.
- `[ ]` **Cost rebalance UI:** when `partial` results show uneven quality (e.g. one worker delivered partials with confidence < 0.5), the orchestrator surfaces a rebalance suggestion in the chain tree view ("Worker X's quality is below threshold — re-award to backup? Cost: $Y"). Owner approves via a one-click confirmation.
- `[ ]` **Pin chain reports:** extend `ChainReportRenderer` with a "Pin" toggle that calls a new RPC `pinChainReport(chainId, pinned: boolean)`. Pinned reports are exempt from the 90-day GC and surface in a dedicated "Pinned" tab.
- `[ ]` **LLM decomposer:** replace the keyword-based `decomposeTask` fallback for plans longer than 3 steps. New module `apps/node/src/llm-chain-decomposer.ts` (~300 lines): takes a goal + mandate, returns a `ChainSubtask[]` with explicit capability tags, depth assignment, and dependency edges. Uses the existing `askPlanner` model-provider surface (no new model dependency).
- `[ ]` **Tests:** `apps/social/test/components/ChainBidInbox.test.tsx`; `apps/social/test/components/ChainCostRebalance.test.tsx`; `apps/node/test/llm-chain-decomposer.test.ts` (golden-plan test against a fixed goal string); multi-bid window expiry test in `chain-orchestrator.test.ts`.
...
### 40E — Cross-orchestrator & cross-home chains `[ ]` deferred until Phase 11 mobile parity ships`

> **Status: explicitly deferred.** Cross-home chains require stable mobile relay endpoints + multi-orchestrator consensus semantics. Layering these on top of an unstable mobile surface is too much area at once. 40E starts after Phase 11 mobile parity ships and Phase 11C–D stabilize.

- `[ ]` Orchestrator hands off a sub-chain to another orchestrator (depth-3 with explicit owner pre-approval).
- `[ ]` Cross-home chains (orchestrator on home-A, workers on home-B and home-C, all reached via relay-only transport on the mobile).
- `[ ]` Cross-network chains via relay-only transport (home node uncontactable directly; only via the relay mesh).
- `[ ]` Cross-orchestrator arbitration when two orchestrators race for the same worker pool (rare; uses existing owner-decision flow).

**Deliverable:** chains that span multiple home nodes + the mobile. This is the precondition for "the agent network truly works as a team."

### Exit Criteria (Phase 40)

- `[ ]` All 40A–40D checkboxes flipped (40E explicitly deferred).
- `[ ]` `npm run typecheck` clean (additive schema bumps; no breaking changes for solo A2A flows).
- `[ ]` `npx vitest run` green — ≥60 new tests across the new files (`chain-budget-ledger`, `chain-orchestrator`, `chain-worker`, `chain-bid-strategy`, `chain-report-synthesizer`, `chain-inbound`, `chain-reports-store`, `agent-network` schemas, four React components).
- `[ ]` Two-node smoke test (`chain-e2e.test.ts`) drives a 3-worker fan-out end-to-end and produces a `ChainReport` on disk with verified `synthesisCostUsd + workerAllocations.committedUsd == maxChainCostUsd` invariant.
- `[ ]` Manual smoke test on a bonded home node + EnvoyGo WebView (chains list visible on mobile; full tree + report on desktop).
- `[ ]` No new npm dependencies (LLM decomposer reuses existing model-provider surface).
- `[ ]` No breaking changes to existing `task.*` wire surface (Phase 24 solo A2A flows continue to work unchanged).
- `[ ]` Trust gating verified per §7.2 directional table (worker→orchestrator bid requires `direct`; orchestrator→worker propose/accept requires `referred`; report→owner uses existing `report.create` channel).
- `[ ]` `bid_expired` audit emitted for stale awards (verified in `chain-worker.test.ts`).
- `[ ]` Crash-replay test (`chain-e2e.test.ts` after a forced orchestrator restart): completed subtasks remain merged; in-flight subtasks are re-bid with `bidExpiresAt` honored.
