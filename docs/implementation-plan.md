...
- `[ ]` **Mobile mirror: deferred.** The original 40C plan called for a read-only "Recent chains" section in `apps/envoygo/lib/screens/me/me_screen.dart` linking to the home node. This is **not implemented** in the 40C/40D/40E scope — EnvoyGo (`apps/envoygo/`) has no chain UI at all (`ls apps/envoygo/lib/screens/` shows only `home | inbox | contacts | chat | me | pairing | terminals | call`). Chains remain a desktop-only UI feature; the 40E relay-wrapped envelopes + the home node's `_homeRemoteCall` delegation mean a future EnvoyGo build can still consume chain reports via `chainListReports` / `chainGetReport` once the home-proxy wire-up lands.
...
- `[x]` `npx vitest run` green for every chain-related test file — **332 chain-related tests across 19 files**:
  ...
  - **74 tests** in 2 protocol/local-store files (`packages/protocol/test/agent-network` 57, `packages/local-store/test/chain-reports-store` 17).
