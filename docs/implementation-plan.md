...
- Video calls (`callType: "video"`) — defer to 42B.
...
- `[ ]` **Tests:** extend `apps/envoygo/test/services/node_service_client_test.dart` — one assertion per method, mocking the `MockWebSocket` JSON-RPC pattern. Verify request payload, response parse, and error propagation.
...
- `[ ]` **Tests:** `apps/envoygo/test/webrtc_call_transport_test.dart` using a fake `RTCPeerConnection` (most `flutter_webrtc` calls are static-method-shaped and mockable; integration tests require a real device). Lifecycle, mute, and ICE candidate buffering on Path 2.

### 42E — Wire `CallProvider` to drive the transport

- `[ ]` **Update** `apps/envoygo/lib/providers/call_provider.dart:99-156`: `startCall` builds a `WebRtcCallTransport`, awaits `startOffer()`, then calls `_nodeService.sendCallInvite(targetOwnerId, sdpOffer, iceServers)` with the generated SDP. `acceptCall` builds a transport from the incoming SDP, awaits `startAnswer()`, then calls `_nodeService.acceptCallInvite(callId, sdpAnswer, iceServers)`. `endCall` / `declineCall` close the transport before the RPC. `toggleMute` calls `transport.setMute` then the RPC. `CallState` gains `MediaStream? remoteStream` and `WebRtcCallTransport? transport` fields.
- `[ ]` **Tests:** `apps/envoygo/test/providers/call_provider_test.dart` — `startCall` builds the transport, generates SDP, sends the RPC, sets state; `acceptCall` does the inverse; transport is closed on every end-state transition.

### 42F — UI + platform config

- `[ ]` **Update** `apps/envoygo/lib/screens/call/voice_call_screen.dart` to render the active call (peer name, duration timer, mute/end buttons) and bind to the new `CallState.remoteStream` via `RTCVideoRenderer` (or audio-only in v1). Update `apps/envoygo/lib/widgets/incoming_call_overlay.dart` to wire the Accept button to `CallProvider.acceptCall` with the SDP plumbing.
- `[ ]` **iOS `Info.plist`:** add `NSMicrophoneUsageDescription`.
- `[ ]` **Android `AndroidManifest.xml`:** add `RECORD_AUDIO` permission.
- `[ ]` **iOS `AVAudioSession` configuration:** `AVAudioSessionCategory.playAndRecord` with `.voiceChat` mode, `.allowBluetooth` option. Set on `startCall` and `acceptCall`, reset on `endCall`.
- `[ ]` **Mic permission:** `permission_handler` `Permission.microphone.request()` before `getUserMedia` if not already granted.
...
- `[ ]` **Playwright E2E (browser-Social ↔ browser-Social):** two browser contexts, place a call, assert `RTCPeerConnection.connectionState === "connected"`. Re-validates the desktop side after the home-side changes. Automates the Phase 38 38H manual smoke.
- `[ ]` **Real-device manual smoke (42H, deferred):** two iOS devices on the same LAN, place a call via EnvoyGo, verify audio. Same pattern as Phase 38H.

### Exit Criteria (Phase 42)

- `[ ]` All 42A–42G sub-phase checkboxes flipped (42H manual device smoke tracked separately, like Phase 38H)
- `[ ]` `npm run typecheck` clean
- `[ ]` `npx vitest run` green (all Phase 38 call tests still pass; new 42A/42B/42G tests pass)
- `[ ]` `flutter analyze` clean on every new/modified `.dart` file
- `[ ]` `flutter test` green for `node_service_client_test.dart`, `call_provider_test.dart`, `webrtc_call_transport_test.dart`, `voice_call_screen_test.dart`
...
- `[ ]` `iceServers` configured on the home reaches the phone via the call envelope (verified by a test that injects a custom `iceServers` into node config and asserts it shows up in the `call.invite` payload)
- `[ ]` Trust gate + identity binding + role policy still enforced on the home (regression-tested from the existing Phase 38 suite)
- `[ ]` iOS `AVAudioSession` config applied on call start, reset on call end; Android `RECORD_AUDIO` permission request fires on first call
- `[ ]` No new npm/yarn/pubspec dependencies (`flutter_webrtc` is already in `pubspec.yaml:28`; verify version compatibility in 42A)
...
|---|----------|--------|-------|
| 1 | `flutter_webrtc` version on EnvoyGo | Open — verify in 42A | `pubspec.yaml:28` declares `^0.12.0`; check whether 0.14 is needed for the API surface used |
| 2 | `iceServers` default list | Open — defaults in 42A | Google STUN only (the existing Phase 38 default) or Google + Cloudflare + Twilio (Phase 38 §6.3 recommendation). Default to the three-list; user can override in node-config |
| 3 | TURN server for symmetric NAT | Open | Phase 38 open question, stays open. 42A documents the workaround: configure public TURN in `iceServers` |
| 4 | Persistent backgrounded calling on iOS | Open | NG8. 42F documents the minimal `AVAudioSession` config; persistent background survival is a separate scope |
...
- `apps/envoygo/lib/providers/call_provider.dart` — existing provider that 42E extends
- `apps/node/src/call-manager.ts` — the home-side state machine
- `apps/node/src/call-inbound.ts` — the home-side trust + identity gate that 42A extends with defensive validators
...
| Date | Change |
|------|--------|
| 2026-06-19 | **Phase 42 — Native WebRTC Voice Calls on EnvoyGo designed.** Phase 38 made desktop↔desktop voice calls real; without Phase 42, the call feature is invisible to mobile. The signaling is already plumbed (home emits `call:*` on the ws event bus; EnvoyGo's `CallProvider` filters by `type.startsWith('call:')`); the media plane and the home's response envelopes are missing. Seven sub-phases (42A–42G) close every gap without changing the wire surface: 42A rewrites `sendCallInvite` to resolve owner→device peer ID via `_resolvePeerTransportForOwner` (currently passes an owner ID, breaking the libp2p dial), embeds the SDP offer (currently empty string), and injects `iceServers` from node-config (currently unread); adds defensive SDP/ICE-candidate validators in `call-inbound.ts` (64 KB SDP cap, SDP candidate grammar regex). 42B wires the four response envelopes (`call.accept` / `call.reject` / `call.hangup` / `call.mute`) so the home actually sends them back to the peer (today the four stubs only mutate local `CallManager` state). 42C replaces the five `UnimplementedError` call RPCs in `NodeServiceClient` with real JSON-RPC. 42D builds the native `WebRtcCallTransport` in Dart using `flutter_webrtc` (Path 2 only; Path 1 libp2p data channel is not available on `flutter_webrtc`). 42E rewires `CallProvider` to drive the transport. 42F lands the active-call screen, mic permission flow, iOS `AVAudioSession` config, and Android `RECORD_AUDIO`. 42G adds the test pyramid (~5 new test files, ~600 LoC, two-`CallManager` jsdom integration, Playwright re-validation of the desktop side). 42H manual two-iOS-device smoke tracked separately like Phase 38H. **No new wire intents, no new schemas, no new npm/pubspec dependencies.** Design doc: [voice-video-call-envoygo.md](./voice-video-call-envoygo.md). |
