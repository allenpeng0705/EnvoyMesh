...
>
> **已知缺口(有意为之):**(1) **Verdict 写入路径** — 权威写入方现在是 orchestrator 的验证流程(Sprint 3,`chain-verify-loop.ts` → `writeVerdictEntry` 写入 `ArbitrationStore`);worker 侧 `adapter.verify` 保持 advisory(§7.1 拒绝自报 reputation),因此 3-tuple 读取器与 `requiresReputationApproval` 会随 verdict 累积而生效。(2) ~~Manifest 池消费者~~ **2026-08-18 已落地(first cut)** — `adapter.manifest` 广播已上线(`agent-adapter-broadcast.ts` + `handleInboundCapabilityManifest` → `ChainSideState.remoteManifests`),`findAgentNetworkWorkersRanked` 优先使用新鲜的 wire manifest 而非卡片合成,`findWorkersWithManifests` / `resolveWorkerPool` 已基于真实 wire 数据生效。加固项(首次接收时 owner 签名校验、manifest TTL 清理)也 **2026-08-18 完成**(`pruneExpiredManifests`,入站 + 读取路径均生效)。
...
- **验证循环接线(`apps/node/src/chain-verify-loop.ts`)— 首版完成 2026-08-18**:`runChainVerificationLoop` 从 `handleOrchestratorPartial` 调用;写 `rule`/`cross` `VerdictEntry` 到 `ArbitrationStore`;按需升级到第二个 runtime。**生产第二医生 seam 已关闭 2026-08-18:** `buildChainOrchestratorDeps` 的 `chainVerify` 现在在 `listRuntimes` 里注册两个本地 runtime,并在 `buildAdapter` 里构造真实 adapter(`openclaw` → `OpenClawAdapter`;`pi` → `createPiAdapterFromHost` → 本地 Pi runtime,两者都用 `buildSubtaskPromptForAdapter` 提示面),因此 OpenClaw worker 会升级到本地 Pi 第二医生。由 `apps/node/test/second-doctor-pi.test.ts` 覆盖(生产 `buildChainOrchestratorDeps` 接线 + 经 `handleOrchestratorPartial` 的真实 Pi 升级)。
...
- **Two-doctor cross-agent**:一条 `criticality: 'high'` 的 chain 跑同一任务,两个不同 runtime,断言 `CrossAgentDisagreementVerifier` 在结果一致时返回 `pass`,不一致时 `disputed`。— **2026-08-18 完成**于三层:adapter-seam(`apps/node/test/side-by-side-harness.test.ts`)、orchestrator 级(`apps/node/test/chain-verify-orchestrator.test.ts`,用生产形状的 verify deps 驱动 `handleOrchestratorPartial`:真实 `ArbitrationStore` 写入、runtime 从 wire manifest 解析、升级到另一个 runtime、verification budget 提交),以及**生产接线级**(`apps/node/test/second-doctor-pi.test.ts`)—— 驱动真实的 `buildChainOrchestratorDeps`,证明 OpenClaw worker 会升级到**真实**的本地 Pi 第二医生。
