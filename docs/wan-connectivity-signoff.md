...
| _pending_ | _release tag_ | Phase 17 geo DHT — `geo-discovery-wan-signoff.test.ts` via cn-relay | `[ ]` same relay as §4 baseline | — | — | _operator_ | `./scripts/wan-relay-signoff-staging.sh` now runs geo signoff after relay chat; record row when green |
...
| `geo-discovery-wan-signoff.test.ts` | Phase 17 — two relay-bootstrap nodes advertise/find `geo:city:*` on live DHT (requires `TEST_RELAY_ADDR`; ~9 min) |

Multi-machine §4–§5 remains **operator-driven** until a stable public relay fleet is available for automated cross-NAT CI. **Automated baseline:** `apps/node/test/wan-relay-signoff-e2e.test.ts` (skipped without `TEST_RELAY_ADDR`). **Geo DHT signoff:** `apps/node/test/geo-discovery-wan-signoff.test.ts` (same env; run via `./scripts/wan-relay-signoff-staging.sh` or nightly). **Staging procedure:** [wan-two-nat-staging-runbook.md](./wan-two-nat-staging-runbook.md) + `./scripts/wan-relay-signoff-staging.sh`.
