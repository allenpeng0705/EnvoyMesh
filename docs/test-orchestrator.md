...
| Mode       | typecheck | unit | social-build | e2e-fast (libp2p) | e2e-playwright (chromium) | smoke | smoke-isolated (libp2p, sequential) | bundle.sh | JUnit | bail |
|------------|:---------:|:----:|:------------:|:------------------:|:-------------------------:|:-----:|:---------:|:-----:|:----:|
| `dev`      | ✓         | ✓    | –            | –                  | –                         | –     | –         | –     | –    |
| `unit`     | ✓         | ✓    | –            | –                  | –                         | –     | –         | –     | –    |
| `e2e`      | –         | –    | ✓            | ✓                  | ✓                         | –     | –         | –     | –    |
...
| 6d | `smoke-isolated`   | three-process + remote-assigner chaos smokes (Phase 13/60/64 libp2p homes, **must not** run in parallel with each other — sequential with 3 s settle window) | ~60s   |
...
- `06d-smoke-isolated-<name>-junit.xml` — three-process + remote-assigner chaos smokes (run one-per-invocation, 3 s between, to avoid libp2p stream / port contention)
