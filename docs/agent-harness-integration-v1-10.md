...
**Total: 30 unit tests** across 5 `describe` blocks
(14 `reputationFromVerdicts` + 8
`categorizeReputation` + 3
`isNoHistoryReputation` + 3
`SCOREBOARD_SOURCE_WEIGHTS` + 2
`SCOREBOARD_TRUST_THRESHOLDS`). The 25-case
test plan above was the original design
spec; the implementation added 5 boundary
tests during self-review (the second
`SCOREBOARD_SOURCE_WEIGHTS` weight-ordering
test + the second `SCOREBOARD_TRUST_THRESHOLDS`
threshold-ordering test + the
`isNoHistoryReputation(100)` boundary case
+ the all-`pass`/`fail`/etc. symmetric
verifications that ensure the formula is
source-agnostic).
