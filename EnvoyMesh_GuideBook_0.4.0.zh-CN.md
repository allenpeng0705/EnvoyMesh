...
**版本类型：** 完整指南版  
**修订日期：** 2026-08-27  
**语言：** [English](EnvoyMesh_GuideBook_0.4.0.md) · [简体中文](EnvoyMesh_GuideBook_0.4.0.zh-CN.md) ([HTML](EnvoyMesh_GuideBook_0.4.0.html) · [中文 HTML](EnvoyMesh_GuideBook_0.4.0.zh-CN.html))  
...
> **完整指南版本。** 本指南反映了修订日期时 EnvoyMesh 0.4.0 存储库的状态。它是为最终用户编写的，而不是作为内容大纲存根。功能状态可能因平台和部署而异 — 在生产环境中依赖它之前，请验证您构建中的每个 Beta 或实验性功能（发行说明、设置标签和附录 J）。
...
#### 20.9 家庭网络 — 一个主节点，多个个人资料

家庭网络将单个 EnvoyMesh 主节点变成一个私有家庭社交网络。所有者安装节点、配置模型并配对手机；然后每个家庭成员配对自己的手机，获得专注、独立的体验 — 自己的个人资料、自己的 AI 线程和家庭直接 + 群组聊天。无云、无订阅，除您配置的 LLM API 调用外没有数据离开您的家。可以把它想象为一个账号上的 Netflix 个人资料，或一台机器上的 macOS 用户账户：共享基础设施，隔离体验。

主节点保持单个网格身份（`envoy:owner:…`）— 所有者是 P2P 网格中唯一可见的参与者。家庭成员作为主节点上的本地个人资料存在，而非网格对等节点；他们的联系人和聊天源自家庭名册，而非网格发现。

#### 20.10 所有者和家庭成员角色

**所有者**保留完整的 EnvoyMesh 产品：EnvoyAI、角色机器人、Ext Agent 聊天、终端、Pi 编码智能体、保险库、外部网格绑定、节点设置和家庭管理（创建、重命名和删除成员个人资料；配置模型 API 密钥和基础设施）。

**家庭成员**获得专注的子集：自己的个人资料、AI 和机器人线程、Ext Agent 聊天、家庭直接和群组聊天、推送通知。他们**无法**使用外部网格绑定、终端、Pi、保险库或节点设置。每个个人资料在配对时锁定到一台设备 — 应用内没有个人资料切换，这让每个人的数据和 AI 线程保持清晰隔离。

#### 20.11 邀请家庭成员

所有者从"设置"创建家庭个人资料并生成**家庭邀请二维码**。这不同于普通的 EnvoyGo 配对二维码：家庭邀请将配对设备绑定到该特定成员个人资料，而普通配对绑定到所有者的完整节点。家庭成员安装 EnvoyGo，扫描邀请二维码，手机通过 WebSocket 或 libp2p 电路中继配对到主节点。配对后，该成员自动出现在每个其他家庭成员的联系人列表中 — 无需单独的绑定步骤。

每个个人资料锁定到一台设备，因此更换手机的家庭成员必须由所有者重新配对（旧设备被撤销）。所有者可以随时重命名或删除个人资料；删除会移除该成员的数据并撤销其设备。

#### 20.12 家庭直接和群组聊天

家庭成员可以互相发送直接消息并参与仅限家庭的群组聊天。这些对话是主节点本地的 — 它们从不遍历更广泛的网格，家庭联系人不是网格对等节点。状态（在线/离开/离线）反映每个成员配对设备到主节点的连接。推送通知（Android 上为 FCM，iOS 上为 APNs）在 EnvoyGo 处于后台时将消息发送到成员的手机，因此家庭聊天感觉就像任何其他消息应用，同时保持在您控制的基础设施上。

#### 20.13 共享 AI 智能体，隔离数据

所有家庭成员共享主节点的模型配置和智能体运行时 — 一个 LLM API 密钥、一个 OpenClaw / Ext Agent 运行时 — 因此所有者配置一次 AI，每个成员都获得助手。但每个成员的 AI 线程、机器人对话和 Ext Agent 聊天对该个人资料是私有的。没有家庭成员可以读取另一个人的 AI 历史记录，成员数据在个人资料级别隔离。共享基础设施，隔离体验：AI 是同一个引擎，但每个人的记忆和对话保持密封。

#### 20.14 家庭成员无法访问的内容

为保持家庭体验专注和安全，成员个人资料被特意缩小范围。家庭成员**无法**：

- 打开到主机的终端（仅限所有者）
- 使用 Pi 编码智能体（仅限所有者）
- 浏览保险库或库（仅限所有者）
- 与外部网格联系人绑定或参与网格发现（仅限所有者）
- 更改节点、中继或模型设置（仅限所有者）
- 管理其他家庭个人资料（仅限所有者）

如果成员需要网格访问、终端或保险库，所有者可以从所有者个人资料代表他们执行操作，或者成员应该在自己的独立主节点上作为所有者配对。家庭网络是私有社交层，而非共享管理控制台。
...
#### 30.6 内容标签页 — Feed、Blog 和 Explore

社交界面和 EnvoyGo 中的**内容**标签页是 `envoy://` 网格内容的用户界面，包含三个子标签：

- **Feed** — 来自您关注的作者和主题的帖子和更新的按时间顺序的社交提要
- **Blog** — 基于 30.1 中的发布模板的长篇文章，在专用阅读器中阅读
- **Explore** — 发现公共和绑定作者、热门主题和市集列表

您在此阅读的所有内容都是内容寻址的，并从绑定联系人的节点点对点交付 — 没有中央内容服务器。打开任何项目可通过网格浏览器（30.2）或 `library.read` 阅读，具体取决于格式。内容标签页不引入新的交付路径；它是本节已记录的内容系统的聚合视图。

#### 30.7 Feed 和 Blog — 阅读和发布

**Feed** 聚合来自您关注的作者和主题的短帖子和共享项目。新帖子作为签名发布事件到达；您的节点验证作者、根据绑定等级检查敏感度并内联渲染。浏览、点赞和评论 — 评论本身是签名网格消息，因此对话可审计并保持在您的信任边界内。没有算法排名：Feed 按时间顺序排列，您通过选择关注谁和关注什么来控制显示内容。

**Blog** 用于长篇内容。使用发布编辑器（30.1）起草带有标题、正文和可选封面媒体的文章；选择可见性等级（公共/朋友/信任）并发布。您的文章作为 `envoy://` 页面存在于您的节点上，并在下次获取时同步到绑定对等节点。关注者在其 Feed 中看到它；其他人可以通过 Explore 找到它。通知重用关注提要机制 — 网格交付不需要单独的推送通道。

#### 30.8 Explore — 发现网格内容

**Explore** 是网格内容的发现层。它以元数据优先的卡片列出公共和绑定作者、热门主题和市集产品 — 标题、作者、敏感度和内容哈希 — 因此您在传输任何字节之前决定获取什么。选择卡片会通过网格浏览器（30.2）或 `library.read` 打开完整内容，具体取决于格式，并经过通常的绑定和敏感度检查。这种元数据优先设计将带宽和暴露降至最低：您看到存在的内容而不会自动拉取它。

Explore 不会绕过信任。公共陌生人内容仍受您的公共敏感度上限限制，推荐作者通过您的绑定图浮现，被阻止的作者永远不会出现。将 Explore 视为网格目录，而非提要 — 它显示可达节点上存在的内容，而不是已推送给您的内容。
...
## 0.3.0 新增与更新（2026-08-27）

本附录补充完整英文指南（`EnvoyMesh_GuideBook_0.4.0.md`）中 0.3.0 版本新增的小节要点。详细步骤请参阅英文版对应章节编号。

### Envoy Harness（Beta）

**Envoy Harness** 是在主节点上运行的编程智能体体验：编辑文件、运行测试、审查 diff、断线后恢复。桌面 Social（Chat 侧栏）与 Terminal 共用语义时间线；EnvoyGo 可监控、批准与回答提问，但不是完整 IDE。高风险工具调用与 EnvoyAI 一样进入主节点审批队列（见英文指南 §25.11–25.13、§28.11、§78.11–78.13）。

### 知识库 — Envoy Local 嵌入与恢复

桌面默认使用主节点上的 **Envoy Local** 嵌入 sidecar（Qwen3-Embedding），无需云端嵌入 API 密钥。若块过大或 sidecar 卡住，**重新索引**会自动缩小并重试、必要时重启嵌入进程，仅跳过仍失败的文档（英文指南 §29.10–29.11）。从旧版本升级后请运行一次 **知识库 → 重新索引**。

### 智能体网络 — 团队任务与推测执行

**团队任务（Team jobs）** 是面向用户的名称；代码与旧文档可能仍称「链」。在链默认设置中启用 **speculation（推测执行）** 时，编排器可对单步并行尝试多种方案，仍受 mandate 预算与审批约束（Beta，§55.11）。EnvoyGo 上的团队任务与 Harness 均为监控/只读或批准为主，完整编排请在桌面 Social 完成（§60.9）。

---

## 0.4.0 新增与更新（2026-09-01）

### Envoy Market（市集）

**Envoy Market** 是主节点上的 **人类店铺与挂牌市场**（英文指南 Part XVI），与智能体网络的「工作者市场」不同。每位主人在 **我的店铺** 创建挂牌（标题、价格、照片、分类），选择 **`public`**（默认，陌生人可搜索）或 **`bonds`**（仅联系人可见），并在 **市集 → 浏览** 搜索全网与绑定店铺的挂牌。

- **询价聊天**：对挂牌发起 **listing-scoped** 会话，无需先加好友；聊得好可再 **加为联系人**。
- **EnvoyAI 短名单**：与 UI 共用搜索 API，可让 EnvoyAI 帮你找 2–3 个选项。
- **状态**：可标记 **预留 / 已售**；**0.4.0 无应用内支付或托管** — 线下成交后更新状态即可。
- **Social 标签页**：动态 · 博客 · **市集** · 发现 · 探索（浏览器）。
...
- 保险库和库
- RAG 知识检索
- **Envoy Market（市集）** — 店铺、挂牌、搜索、询价聊天
- 团队任务
- 终端访问
- Envoy Harness（Beta，桌面 Social + Terminal）
- 浏览器（基本功能）
- 私人和社区中继
